...
if (isset($_SESSION['admin_id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
        ...
        // Fetch current admin data to retain unupdated fields
        $stmt = $conn->prepare("SELECT first_name, last_name, email, phone, profile_image FROM admins WHERE id = ?");
        ...
        if ($current_admin) {
            ...
            $first_name = $first_name ?? $current_admin['first_name'];
            $last_name = $last_name ?? $current_admin['last_name'];
            $email = $email ?? $current_admin['email'];
            $phone = $phone ?? $current_admin['phone'];
            $profile_image = $profile_image ?? $current_admin['profile_image'];

            // Update the database
            $update_stmt = $conn->prepare("UPDATE admins SET first_name = ?, last_name = ?, email = ?, phone = ?, profile_image = ? WHERE id = ?");
