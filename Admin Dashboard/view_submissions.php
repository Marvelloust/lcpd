...
if (isset($_SESSION['admin_id'])) {
    ...
    if ($assignment) {
        // Fetch the submitted answers for the assignment
        $stmt = $conn->prepare("SELECT * FROM answers WHERE assignment_id = ?");
        $stmt->bind_param("i", $assignment_id);
        $stmt->execute();
        $result = $stmt->get_result();
    }
        ...
        <div class="dashboard">
            ...
            <div class="main-content" id="main-content">
                <header>
                    ...
                    <h2>Submissions for Assignment: <?= htmlspecialchars($assignment['assignment_title']); ?></h2>
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php while ($answer = $result->fetch_assoc()): ?>
                                ...
                                $stmt->bind_param("i", $answer['user_id']);
                                $stmt->execute();
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        $stmt = $conn->prepare("SELECT * FROM grades WHERE assignment_id = ? AND user_id = ?");
                                        $stmt->bind_param("ii", $assignment_id, $answer['user_id']);
                                        $stmt->execute();
                                        ...
                                        <?php if ($grade): ?>
                                            ...
                                            <form action="award_grade.php" method="post">
                                        ...
                                        <?php else: ?>
                                            <!-- Form to award the grade -->
                                            <form action="award_grade.php" method="post">
                                                <input type="hidden" name="assignment_id" value="<?= $assignment_id; ?>">
                                                <input type="hidden" name="user_id" value="<?= $answer['user_id']; ?>">
                                                <input type="number" name="grade" placeholder="Enter grade">
                                                <button type="submit">Award Grade</button>
                                            </form>
                                        <?php endif; ?>
                                    </td>
                                </tr>
