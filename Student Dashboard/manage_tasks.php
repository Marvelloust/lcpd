...
if ($action === 'add') {
    ...
    $stmt = $conn->prepare("INSERT INTO tasks (task_title, task_date, task_duration, user_id) VALUES (?, ?, ?, ?)");
    ...
    $stmt = $conn->prepare("UPDATE tasks SET task_title = ?, task_date = ?, task_duration = ? WHERE id = ? AND user_id = ?");
