<?php
// Start the session
session_start();

// Assuming you have stored the user's ID in the session during login
if (isset($_SESSION['user_id'])) {
    $user_id = $_SESSION['user_id'];

    // Connect to the database
    include 'db_connection.php';

    // Fetch the logged-in user's details from the database
    $stmt = $conn->prepare("SELECT first_name, last_name, profile_image FROM users WHERE id = ?");
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
    $result = $stmt->get_result();

    // If the user is found
    if ($result->num_rows > 0) {
        $user = $result->fetch_assoc();
        $first_name = $user['first_name'];
        $last_name = $user['last_name'];
        // Check if the profile image exists; if not, use a default image
        $profile_image = !empty($user['profile_image']) ? $user['profile_image'] : 'data:image/png;base64,'; // Default image if none is set
    } else {
        // If no user is found, redirect to login or show an error
        header("Location: ../login.php");
        exit();
    }
} else {
    // If no session is set, redirect to login
    header("Location: ../login.php");
    exit();
}
?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Student Dashboard</title>
    <link rel="stylesheet" href="css/styles.css">
    <link rel="stylesheet" href="css/students.css">
    <!-- Optional: Include Google Fonts for better typography -->
    <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@400;500;700&display=swap" rel="stylesheet">
    <link href='https://unpkg.com/boxicons@2.1.4/css/boxicons.min.css' rel='stylesheet'>
    <link rel="shortcut icon" href="../img/licon.png" type="image/x-icon">
</head>

<body>
    <div class="dashboard">
        <!-- Sidebar -->
        <?php include 'sidebar.php' ?>

        <!-- Main Content -->
        <div class="main-content" id="main-content">
            <header>
                <button class="toggle-btn" id="toggle-btn">&#9776;</button>
                <h2>Uploaded Assignments</h2>
                <div class="user-wrapper">
                    <img src="<?php echo $profile_image; ?>" alt="User Image" style="width: 40px; height: 40px; border-raduis: 50%; object-fit: cover;">
                    <div>
                        <h4><?php echo $first_name . ' ' . $last_name; ?></h4>
                        <small>Super user</small>
                    </div>
                </div>
            </header>

            <!-- Display Uploaded Assignments -->
            <div class="uploaded-assignments">
                <table>
                    <thead>
                        <tr>
                            <th>Assignment Title</th>
                            <th>Course Title</th>
                            <th>Assignment File</th>
                            <th>Uploaded At</th>
                            <th>Submit Answer</th>
                        </tr>
                    </thead>
                    <?php
                    // Fetch all uploaded assignments from the database for the student's enrolled courses
                    $stmt = $conn->prepare("SELECT a.id, a.assignment_title, c.course_title, a.assignment_file, a.uploaded_at, s.status FROM assignments a JOIN courses c ON a.course_id = c.id JOIN students s ON c.id = s.course_id WHERE s.user_id = ? AND s.status = 'Approved'");
                    $stmt->bind_param("i", $user_id);
                    $stmt->execute();
                    $result = $stmt->get_result();

                    // Display the uploaded assignments in the table
                    while ($assignment = $result->fetch_assoc()) {
                        echo '<tr>';
                        echo '<td>' . $assignment['assignment_title'] . '</td>';
                        echo '<td>' . $assignment['course_title'] . '</td>';
                        echo '<td><a href="' . $assignment['assignment_file'] . '">Download</a></td>';
                        echo '<td>' . date('D j Y', strtotime($assignment['uploaded_at'])) . '</td>';

                        // Check if the user has already submitted an answer for the assignment
                        $stmt = $conn->prepare("SELECT * FROM answers WHERE user_id = ? AND assignment_id = ?");
                        $stmt->bind_param("is", $user_id, $assignment['id']);
                        $stmt->execute();
                        $result2 = $stmt->get_result();

                        if ($result2->num_rows > 0) {
                            // If the user has already submitted an answer, disable the submit button
                            echo '<td><button type="button" class="btn btn-primary" disabled>Already Submitted</button></td>';
                        } else {
                            // If the user has not submitted an answer, display the submit button
                            echo '<td><a href="submit_answer.php?assignment_id=' . $assignment['id'] . '">Submit Answer</a></td>';
                        }

                        echo '</tr>';
                    }
                    ?>
                </table>
            </div>
        </div>
    </div>

    <script src="js/scripts.js"></script>
</body>

</html>