...
if (isset($_SESSION['user_id'])) {
    ...
    // Total approved and pending courses
    $stmt = $conn->prepare("SELECT COUNT(*) as total_courses FROM students WHERE user_id = ? AND status = 'Approved'");
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
    $approved_courses = $stmt->get_result()->fetch_assoc()['total_courses'];

    $stmt = $conn->prepare("SELECT COUNT(*) as total_courses FROM students WHERE user_id = ? AND status = 'Pending'");
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
    $pending_courses = $stmt->get_result()->fetch_assoc()['total_courses'];

    // Total grades
    $stmt = $conn->prepare("SELECT SUM(grade) as total_grade FROM grades WHERE user_id = ?");
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
    $total_grade = $stmt->get_result()->fetch_assoc()['total_grade'] ?? 0;
    ...
    // Total assignments
    $stmt = $conn->prepare("SELECT COUNT(*) as total_assignments FROM assignments WHERE course_id IN (SELECT course_id FROM students WHERE user_id = ?)");
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
